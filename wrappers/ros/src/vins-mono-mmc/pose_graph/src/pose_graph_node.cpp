...
void command()
{
    ...
    while(1)
    {
        ...
        if (c == 's')
        {
            ...
            ros::shutdown();
            cout<<"program is shutted down."<<endl;
        }
        if (c == 'n')
            new_sequence();
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_imu_forward = n.subscribe("/vins_estimator/imu_propagate", 100, imu_forward_callback);
    ...
    ros::Subscriber sub_pose1 = n.subscribe("/vins_estimator/saved_keyframe", 100, pose_callback);
    ...
    ros::Subscriber sub_relo_relative_pose = n.subscribe("/vins_estimator/relo_relative_pose", 100, relo_relative_pose_callback);

    pub_match_img = n.advertise<sensor_msgs::Image>("match_image", 100);
    pub_camera_pose_visual = n.advertise<visualization_msgs::MarkerArray>("camera_pose_visual", 100);
    pub_key_odometrys = n.advertise<visualization_msgs::Marker>("key_odometrys", 100);
    pub_vio_path = n.advertise<nav_msgs::Path>("no_loop_path", 100);
